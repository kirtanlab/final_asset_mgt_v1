...
const ClassificationDetailsTableRow = ({
  ...
  const [editClassifications, setEditClassifications] = useState(false);
  const popover = usePopover();
  const rowConfirm = useBoolean();
  const deleteClassificationWithIds = useDeleteClassificationWithIds();
  ...
  return (
    <TableRow hover selected={selected}>
      ...
      <TableCell>
        <ListItemText
          primary={row?.location?.location_name}
          primaryTypographyProps={{ typography: 'body2', noWrap: false }}
          secondaryTypographyProps={{
            mt: 0.5,
            component: 'span',
      ...
      <TableCell>
        <ListItemText
          primary={row?.owner?.employee_name}
          primaryTypographyProps={{ typography: 'body2', noWrap: false }}
          secondaryTypographyProps={{
            mt: 0.5,
            component: 'span',
      ...
      <CustomDialog
        openFlag={editClassifications}
        setonClose={() => setEditClassifications(false)}
        placeHolder="Edit Classifications"
        component={<EditClassifications row={row} />}
      />
      <ConfirmDialog
        open={confirm.value}
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                console.log('obj', obj);
                await deleteClassificationWithIds.mutateAsync(obj);
                confirm.onFalse();
                table.setSelected([]);
              } catch (error) {
...
ClassificationDetailsTableRow.propTypes = {
...
export default ClassificationDetailsTableRow;
