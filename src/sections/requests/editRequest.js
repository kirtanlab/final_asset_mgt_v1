...
function EditAssetRequest({ row, type }) {
  const { user } = useAuth();
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      reporting_to_id: row?.reportingToId?.employee_name || '',
    }),
    [row]
  );
  const today = dayjs();
    ...
    try {
      ...
      if (type === 'edit') {
        ...
        obj = {
          ...
          reportingToId: AllEmployeeData.data.find(
        ...
        <Card sx={{ p: 3 }}>
          ...
          >
            ...
            <RHFAutocomplete
              ...
              label="Category *"
              disabled={type === 'view' ? true : undefined}
              options={getAllCategoriesData.data
                .filter((x) => x?.status === true)
            ...
            <RHFAutocomplete
              ...
              disabled={type === 'view' ? true : undefined}
              options={getAllClassificationData.data
                .filter((x) => x?.status === true)
                .map((x) => x?.class_name)}
              getOptionLabel={(option) => option}
              isOptionEqualToValue={(option, value) => option === value}
            ...
            <RHFAutocomplete
              ...
              options={AllEmployeeData.data
                .filter((x) => x?.role === 'manager' && x?.status === true)
                .map((x) => x?.employee_name)}
              isOptionEqualToValue={(option, value) => option === value}
              renderOption={(props, option) => {
                const manager = AllEmployeeData.data.find(
                  (x) => x.employee_name === option && x.role === 'manager'
                );
